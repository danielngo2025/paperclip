...
Example Nexio plugin that demonstrates:
...
```bash
pnpm --filter @nexioai/plugin-file-browser-example build
pnpm nexioai plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm nexioai plugin uninstall nexio-file-browser-example --force
```
...
- Optional: use `nexio-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
