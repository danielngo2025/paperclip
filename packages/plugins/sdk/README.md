...
Official TypeScript SDK for Nexio plugin authors.

- **Worker SDK:** `@nexioai/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@nexioai/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@nexioai/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@nexioai/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@nexioai/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@nexioai/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@nexioai/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@nexioai/plugin-sdk/ui/hooks` | Hooks only |
| `@nexioai/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@nexioai/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@nexioai/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@nexioai/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@nexioai/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@nexioai/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main Nexio app. They can call ordinary Nexio HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Nexio instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@nexioai/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@nexioai/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Nexio information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@nexioai/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@nexioai/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@nexioai/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@nexioai/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@nexioai/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@nexioai/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@nexioai/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@nexioai/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@nexioai/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@nexioai/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@nexioai/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@nexioai/plugin-sdk/bundlers";
...
```bash
nexio-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@nexioai/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__nexio__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__nexio__/events` streams `reload` SSE events on UI build changes
