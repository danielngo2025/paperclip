 ...
 * channel.
 *
 * The Nexio plugin runtime uses JSON-RPC 2.0 over stdio to communicate
...
export type { PluginLauncherRenderContextSnapshot } from "@nexioai/shared";
 ...
 * we use strings (UUIDs or monotonic counters) for all Nexio messages.
 */
...
/**
 * Nexio plugin-specific error codes.
 *
...
export interface InitializeParams {
  ...
  instanceInfo: {
    /** UUID of this Nexio instance. */
    instanceId: string;
    /** Semver version of the running Nexio host. */
    hostVersion: string;
  };
  /** Host API version. */
  apiVersion: number;
