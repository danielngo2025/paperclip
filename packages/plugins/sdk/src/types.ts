/**
 * Core types for the Nexio plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@nexioai/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @nexioai/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Nexio object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Nexio UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Nexio domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Nexio domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Nexio secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Nexio secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@nexioai/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@nexioai/plugin-sdk";
 *
