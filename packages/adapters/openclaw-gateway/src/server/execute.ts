...
import { asNumber, asString, buildNexioEnv, parseObject } from "@nexioai/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "nexio";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "nexio";
...
function resolveNexioApiUrlOverride(value: unknown): string | null {
...
function buildNexioEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const paperclipApiUrlOverride = resolveNexioApiUrlOverride(ctx.config.nexioApiUrl);
  const paperclipEnv: Record<string, string> = {
    ...buildNexioEnv(ctx.agent),
    NEXIO_RUN_ID: ctx.runId,
  };

  if (paperclipApiUrlOverride) {
    paperclipEnv.NEXIO_API_URL = paperclipApiUrlOverride;
  }
  if (wakePayload.taskId) paperclipEnv.NEXIO_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) paperclipEnv.NEXIO_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) paperclipEnv.NEXIO_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) paperclipEnv.NEXIO_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) paperclipEnv.NEXIO_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    paperclipEnv.NEXIO_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }
...
function buildWakeText(payload: WakePayload, paperclipEnv: Record<string, string>): string {
  ...
  const orderedKeys = [
    ...
    "NEXIO_TASK_ID",
    "NEXIO_WAKE_REASON",
    "NEXIO_WAKE_COMMENT_ID",
    "NEXIO_APPROVAL_ID",
    "NEXIO_APPROVAL_STATUS",
    "NEXIO_LINKED_ISSUE_IDS",
  ];
  ...
  const apiBaseHint = paperclipEnv.NEXIO_API_URL ?? "<set NEXIO_API_URL>";

  const lines = [
    "Nexio wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `NEXIO_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load NEXIO_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $NEXIO_API_KEY on every API call.",
    "- Use X-Nexio-Run-Id: $NEXIO_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: NEXIO_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$NEXIO_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$NEXIO_COMPANY_ID/issues?assigneeAgentId=$NEXIO_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardNexioPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  paperclipEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateNexio = parseObject(payloadTemplate.nexio);
  const workspace = asRecord(ctx.context.nexioWorkspace);
  const workspaces = Array.isArray(ctx.context.nexioWorkspaces)
    ? ctx.context.nexioWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.nexioRuntimeServiceIntents)
    ? ctx.context.nexioRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardNexio: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: paperclipEnv.NEXIO_API_URL ?? null,
  };

  if (workspace) {
    standardNexio.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardNexio.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardNexio.workspaceRuntime = {
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const paperclipEnv = buildNexioEnvForWake(ctx, wakePayload);
  ...
  const paperclipPayload = buildStandardNexioPayload(ctx, wakePayload, paperclipEnv, payloadTemplate);
