...
## Scope
This plan is now **gateway-only**. Nexio supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Nexio skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Nexio task.
...
```bash
pnpm --filter @nexioai/server test:run -- openclaw-gateway
pnpm --filter @nexioai/server typecheck
pnpm --filter @nexioai/ui typecheck
pnpm --filter nexioai typecheck
```
