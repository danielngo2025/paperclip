...
import type { AdapterModel } from "@nexioai/adapter-utils";
import { asString, runChildProcess } from "@nexioai/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.NEXIO_PI_COMMAND === "string" &&
    process.env.NEXIO_PI_COMMAND.trim().length > 0
      ? process.env.NEXIO_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["NEXIO_", "npm_", "NPM_"] as const;
