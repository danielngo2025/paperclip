...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "nexio-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function resolveNexioHomeDir(): string {
  const envHome = process.env.NEXIO_HOME?.trim();
  if (envHome) return path.resolve(expandHomePrefix(envHome));
  return path.resolve(os.homedir(), ".paperclip");
}

function resolveNexioInstanceId(): string {
...
function resolveDefaultEmbeddedPostgresDir(): string {
  return path.resolve(resolveNexioHomeDir(), "instances", resolveNexioInstanceId(), "db");
}
...
function resolveNexioConfigPath(): string {
  if (process.env.NEXIO_CONFIG?.trim()) {
    return path.resolve(process.env.NEXIO_CONFIG.trim());
  }
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath();
}

function resolveNexioEnvPath(configPath: string): string {
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveNexioEnvPath(configPath);
