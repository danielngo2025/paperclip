...
function resolveNexioHomeDir(): string {
  ...
  return path.resolve(os.homedir(), ".nexio");
}

function resolveNexioInstanceId(): string {
  const raw = process.env.NEXIO_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveNexioHomeDir(), "instances", resolveNexioInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveNexioHomeDir(), "instances", resolveNexioInstanceId(), "data", "backups");
}
