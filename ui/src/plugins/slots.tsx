 ...
 *
 * Plugin bundles are built with `external: ["@nexioai/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__nexioPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__nexioPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__nexioPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__nexioPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@nexioai/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@nexioai/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@nexioai/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@nexioai/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@nexioai/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__nexioPluginBridge__) {
 ...
 * self-register via `window.nexioPlugins.registerReactComponent()`.
 ...
 * of `@nexioai/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
