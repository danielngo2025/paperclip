...
type NexioSprite = (typeof NEXIO_SPRITES)[number];
...
function spriteSize(sprite: NexioSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = NEXIO_SPRITES[(baseCol + baseRow) % NEXIO_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = NEXIO_SPRITES[Math.floor(Math.random() * NEXIO_SPRITES.length)]!;
