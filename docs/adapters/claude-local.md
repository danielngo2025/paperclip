...
The adapter creates a temporary directory with symlinks to Nexio skills and passes it via `--add-dir`. This makes skills discoverable without polluting the agent's working directory.
...
This installs Nexio skills in `~/.claude/skills`, creates an agent API key, and prints shell exports to run as that agent.
