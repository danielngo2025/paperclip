...
Build a custom adapter to connect Nexio to any agent runtime.
...
2. Build environment with `buildNexioEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `nexioai run --watch` using `picocolors`.
...
Make Nexio skills discoverable to your agent runtime without writing to the agent's working directory:
