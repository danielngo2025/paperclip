# ClipHub: Marketplace for Nexio Team Configurations

> The "app store" for whole-company AI teams — pre-built Nexio configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Nexio-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Nexio company (agents, projects, governance) |
...
A complete Nexio company configuration:
...
Single-agent configurations designed to plug into a Nexio org:
...
Portable skill files that any Nexio agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Nexio's skill loading system
...
interface Listing {
  ...
  nexioVersionMin: string;     // Minimum Nexio version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Nexio company
...
| `POST` | `/api/listings/:id/install` | Install to Nexio company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Nexio company (or create new)
  → ClipHub API calls Nexio API to:
  ...
  → Redirect to Nexio dashboard with new team running
```
...
### 5.3 Creator: Export from Nexio → Publish

```
Running Nexio company → "Export as Blueprint" (CLI or UI)
  → Nexio exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Nexio brand blue/purple)
...
| Install flow | One-click deploy to Nexio company |
...
{
  "targetCompanyId": "uuid",         // Existing Nexio company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `nexio-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Nexio
- **Backend**: Node.js API (or extend Nexio server)
- **Database**: Postgres (can share Nexio's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Nexio auth (or OAuth2)

### 9.2 Integration with Nexio

ClipHub can be:
- **Option A**: A separate app that calls Nexio's API to install blueprints
- **Option B**: A built-in section of the Nexio UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Nexio UI and API.
...
- [ ] Install flow: blueprint → Nexio company
