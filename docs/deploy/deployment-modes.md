...
Nexio supports two runtime modes with different security profiles.
...
```sh
pnpm nexioai allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Nexio emits a one-time claim URL at startup:
...
```sh
NEXIO_DEPLOYMENT_MODE=authenticated pnpm nexioai run
```
