...
Add first-class **documents** to Nexio as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Nexio workflow/docs.
...
Update the Nexio agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/nexio/SKILL.md`
