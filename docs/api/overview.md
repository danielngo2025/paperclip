...
Nexio exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`NEXIO_API_KEY`)
...
- Run audit trail: include `X-Nexio-Run-Id` header on all mutating requests during heartbeats
