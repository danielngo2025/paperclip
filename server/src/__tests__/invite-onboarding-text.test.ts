...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Nexio OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested Nexio base URLs to try");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.nexioApiUrl");
    expect(text).toContain("~/.openclaw/workspace/paperclip-claimed-api-key.json");
    expect(text).toContain("NEXIO_API_KEY");
