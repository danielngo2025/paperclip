...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "nexio", LOGNAME: "nexio", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "nexio" }),
      },
    );

    expect(tokens).toEqual(["nexio", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "nexio" }, {
        userInfo: () => ({ username: "nexio" }),
      });

      expect(tokens).toEqual(["nexio", "custom-token"]);
    } finally {
