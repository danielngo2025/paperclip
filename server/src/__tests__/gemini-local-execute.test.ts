...
import { execute } from "@nexioai/adapter-gemini-local/server";
...
describe("gemini execute", () => {
  it("passes prompt as final argument and injects paperclip env vars", async () => {
    ...
    try {
      ...
      expect(capture.argv.at(-1)).toContain("Nexio runtime note:");
      ...
      expect(invocationPrompt).toContain("Nexio runtime note:");
      expect(invocationPrompt).toContain("NEXIO_API_URL");
      expect(invocationPrompt).toContain("Nexio API access note:");
    ...
    try {
      await execute({
        ...
        config: {
          command: commandPath,
          cwd: workspace,
          env: { NEXIO_TEST_CAPTURE_PATH: capturePath },
        },
        context: {},
        authToken: "t",
        onLog: async () => {},
