...
import { ensureCursorSkillsInjected } from "@nexioai/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing Nexio skills into Cursor skills home", async () => {
    ...
    await createSkillDir(skillsDir, "nexio");
    ...
    const injectedA = path.join(skillsHome, "nexio");
    ...
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "nexio")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "paperclip-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "nexio"'))).toBe(true);
  ...
  it("preserves existing targets and only links missing skills", async () => {
    ...
    const existingTarget = path.join(skillsHome, "nexio");
