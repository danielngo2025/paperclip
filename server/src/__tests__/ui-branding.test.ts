...
<head>
    <!-- NEXIO_RUNTIME_BRANDING_START -->
    <!-- NEXIO_RUNTIME_BRANDING_END -->
    <!-- NEXIO_FAVICON_START -->
...
describe("ui branding", () => {
  it("detects worktree mode from NEXIO_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ NEXIO_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ NEXIO_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ NEXIO_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      NEXIO_IN_WORKTREE: "true",
      NEXIO_WORKTREE_NAME: "paperclip-pr-432",
      NEXIO_WORKTREE_COLOR: "#4f86f7",
    });
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="nexio-worktree-name"');
    expect(meta).toContain('content="paperclip-pr-432"');
    expect(meta).toContain('name="nexio-worktree-color"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="nexio-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="nexio-worktree-name"');
  });
});
