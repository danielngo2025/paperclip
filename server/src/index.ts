...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.NEXIO_SECRETS_PROVIDER === undefined) {
    process.env.NEXIO_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.NEXIO_SECRETS_STRICT_MODE === undefined) {
    process.env.NEXIO_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.NEXIO_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.NEXIO_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.NEXIO_MIGRATION_PROMPT === "never") return false;
    if (process.env.NEXIO_MIGRATION_AUTO_APPLY === "true") return true;
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@nexio.local";
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.NEXIO_EMBEDDED_POSTGRES_VERBOSE === "true";
  ...
  if (config.deploymentMode === "authenticated") {
    ...
    const betterAuthSecret =
      process.env.BETTER_AUTH_SECRET?.trim() ?? process.env.NEXIO_AGENT_JWT_SECRET?.trim();
    if (!betterAuthSecret) {
      throw new Error(
        "authenticated mode requires BETTER_AUTH_SECRET (or NEXIO_AGENT_JWT_SECRET) to be set",
      );
    }
  ...
  const runtimeApiHost =
    runtimeListenHost === "0.0.0.0" || runtimeListenHost === "::"
      ? "localhost"
      : runtimeListenHost;
  process.env.NEXIO_LISTEN_HOST = runtimeListenHost;
  process.env.NEXIO_LISTEN_PORT = String(listenPort);
  process.env.NEXIO_API_URL = `http://${runtimeApiHost}:${listenPort}`;
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      if (process.env.NEXIO_OPEN_ON_LISTEN === "true") {
  ...
  return {
    ...
    host: config.host,
    listenPort,
    apiUrl: process.env.NEXIO_API_URL ?? `http://${runtimeApiHost}:${listenPort}`,
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "Nexio server failed to start");
    process.exit(1);
  });
}
