...
import type { DeploymentExposure, DeploymentMode } from "@nexioai/shared";
import {
  forbidden,
  conflict,
  notFound,
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.nexioApiUrl)) {
    const legacyNexioApiUrl = nonEmptyTrimmedString(input.nexioApiUrl);
    if (legacyNexioApiUrl) merged.nexioApiUrl = legacyNexioApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    nexioApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.nexioApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawNexioApiUrl =
    typeof defaults.nexioApiUrl === "string"
      ? defaults.nexioApiUrl.trim()
      : "";
  if (rawNexioApiUrl) {
    try {
      const parsedNexioApiUrl = new URL(rawNexioApiUrl);
      if (
        parsedNexioApiUrl.protocol !== "http:" &&
        parsedNexioApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_nexio_api_url_protocol",
          level: "warn",
          message: `nexioApiUrl must use http:// or https:// (got ${parsedNexioApiUrl.protocol}).`
        });
      } else {
        normalized.nexioApiUrl = parsedNexioApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_nexio_api_url_configured",
          level: "info",
          message: `nexioApiUrl set to ${parsedNexioApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_nexio_api_url_invalid",
        level: "warn",
        message: `Invalid nexioApiUrl: ${rawNexioApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Nexio host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Nexio is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm nexioai allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/paperclip-claimed-api-key.json and load NEXIO_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: nexioApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Nexio hostname is reachable and allowed via `pnpm nexioai allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Nexio API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Nexio OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          nexioApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Nexio generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "nexioApiUrl": "https://paperclip-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Nexio before key claim is allowed.
    ...
    And set the NEXIO_API_KEY and NEXIO_API_URL in your environment variables as specified here:
    ...
    Then set NEXIO_API_KEY and NEXIO_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Nexio skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your NEXIO_API_URL to the top of your skill and note the path to your NEXIO_API_URL
    ...
    ${
      ...
      "Ensure Nexio is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Nexio base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.nexioApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm nexioai allowed-hostname <host>
      - then restart Nexio and retry onboarding.
    `);
  }
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "nexio", path: "/api/skills/paperclip" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "nexio-create-agent",
          path: "/api/skills/nexio-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              nexioApiUrl: req.body.nexioApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.nexioApiUrl &&
          !persistedDefaults.nexioApiUrl
        ) {
          missingPersistedFields.push("nexioApiUrl");
        }
