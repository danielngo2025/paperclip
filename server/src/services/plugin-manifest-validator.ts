 ...
 */
import { pluginManifestV1Schema } from "@nexioai/shared";
import type { NexioPluginManifestV1 } from "@nexioai/shared";
import { PLUGIN_API_VERSION } from "@nexioai/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): NexioPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as NexioPluginManifestV1,
        };
      }
