...
import type { NexioPluginManifestV1 } from "@nexioai/shared";
...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __nexio_exports: module.exports,
      __nexio_module: module,
      __nexio_require: requireInSandbox,
      __nexio_filename: realPath,
      __nexio_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__nexio_exports, __nexio_module, __nexio_require, __nexio_filename, __nexio_dirname)`;
