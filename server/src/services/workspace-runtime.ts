...
import type { AdapterRuntimeServiceReport } from "@nexioai/adapter-utils";
import type { Db } from "@nexioai/db";
import { workspaceRuntimeServices } from "@nexioai/db";
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.NEXIO_WORKSPACE_CWD = input.worktreePath;
  env.NEXIO_WORKSPACE_PATH = input.worktreePath;
  env.NEXIO_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.NEXIO_WORKSPACE_BRANCH = input.branchName;
  env.NEXIO_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.NEXIO_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.NEXIO_WORKSPACE_SOURCE = input.base.source;
  env.NEXIO_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.NEXIO_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.NEXIO_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.NEXIO_PROJECT_ID = input.base.projectId ?? "";
  env.NEXIO_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.NEXIO_AGENT_ID = input.agent.id;
  env.NEXIO_AGENT_NAME = input.agent.name;
  env.NEXIO_COMPANY_ID = input.agent.companyId;
  env.NEXIO_ISSUE_ID = input.issue?.id ?? "";
  env.NEXIO_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.NEXIO_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".nexio", "worktrees");
