...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@nexioai/db";
 ...
 *
 * This implementation delegates to the core Nexio domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@nexioai/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
