...
function jwtConfig() {
  const secret = process.env.NEXIO_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.NEXIO_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.NEXIO_AGENT_JWT_ISSUER ?? "nexio",
    audience: process.env.NEXIO_AGENT_JWT_AUDIENCE ?? "nexio-api",
  };
}
