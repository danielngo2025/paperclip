const FAVICON_BLOCK_START = "<!-- NEXIO_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- NEXIO_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- NEXIO_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- NEXIO_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.NEXIO_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.NEXIO_WORKTREE_NAME) ?? nonEmpty(env.NEXIO_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.NEXIO_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="nexio-worktree-enabled" content="true" />',
    `<meta name="nexio-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="nexio-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="nexio-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
