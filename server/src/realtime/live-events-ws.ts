...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@nexioai/db";
import type { DeploymentMode } from "@nexioai/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  nexioUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).nexioUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.nexioUpgradeContext = context;
