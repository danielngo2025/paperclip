...
name: release
description: >
  Coordinate a full Nexio release across engineering verification, npm,
...
Run the full Nexio release as a maintainer workflow, not just an npm publish.
...
7. If running through Nexio, you have issue context for status updates and follow-up task creation.
...
Nexio now uses this release model:
...
```bash
npm view nexioai@canary version
```
...
- any release summary in Nexio issue context
