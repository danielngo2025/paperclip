...
2. **Build environment** — call `buildPaperclipEnv(agent)` then layer in `NEXIO_RUN_ID`, context vars (`NEXIO_TASK_ID`, `NEXIO_WAKE_REASON`, `NEXIO_WAKE_COMMENT_ID`, `NEXIO_APPROVAL_ID`, `NEXIO_APPROVAL_STATUS`, `NEXIO_LINKED_ISSUE_IDS`), user env overrides, and auth token
...
| `buildPaperclipEnv(agent)` | Standard `NEXIO_*` env vars |
