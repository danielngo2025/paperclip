...
Environment overrides:
  ...
  NEXIO_HOST=127.0.0.1    Host passed to the onboarded server
  NEXIO_PORT=3232         Port passed to the onboarded server
...
export NEXIO_HOST="${NEXIO_HOST:-127.0.0.1}"
export NEXIO_PORT="${NEXIO_PORT:-3100}"
export NEXIO_OPEN_ON_LISTEN="${NEXIO_OPEN_ON_LISTEN:-false}"
...
echo "NEXIO_HOST: $NEXIO_HOST"
echo "NEXIO_PORT: $NEXIO_PORT"
...
env \
  ...
  HOST="$NEXIO_HOST" \
  PORT="$NEXIO_PORT" \
  NEXIO_OPEN_ON_LISTEN="$NEXIO_OPEN_ON_LISTEN" \
