...
NEXIO_DEPLOYMENT_MODE="${NEXIO_DEPLOYMENT_MODE:-authenticated}"
NEXIO_DEPLOYMENT_EXPOSURE="${NEXIO_DEPLOYMENT_EXPOSURE:-private}"
NEXIO_PUBLIC_URL="${NEXIO_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e NEXIO_DEPLOYMENT_MODE="$NEXIO_DEPLOYMENT_MODE" \
      -e NEXIO_DEPLOYMENT_EXPOSURE="$NEXIO_DEPLOYMENT_EXPOSURE" \
      -e NEXIO_PUBLIC_URL="$NEXIO_PUBLIC_URL" \
      -e NEXIO_HOME="/paperclip" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "paperclipai@${PAPERCLIPAI_VERSION}" auth bootstrap-ceo --data-dir "$NEXIO_HOME" --base-url "$NEXIO_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$NEXIO_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$NEXIO_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$NEXIO_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$NEXIO_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$NEXIO_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$NEXIO_PUBLIC_URL/api/companies")"
...
echo "    Public URL: $NEXIO_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $NEXIO_DEPLOYMENT_MODE/$NEXIO_DEPLOYMENT_EXPOSURE"
...
if ! wait_for_http "$NEXIO_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $NEXIO_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$NEXIO_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
