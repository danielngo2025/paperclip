...
NEXIO_API_URL="${NEXIO_API_URL:-http://127.0.0.1:3100}"
API_BASE="${NEXIO_API_URL%/}/api"
...
NEXIO_API_URL_FOR_OPENCLAW="${NEXIO_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${NEXIO_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${NEXIO_AUTH_HEADER}" )
fi
if [[ -n "${NEXIO_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${NEXIO_COOKIE}" )
  NEXIO_BROWSER_ORIGIN="${NEXIO_BROWSER_ORIGIN:-${NEXIO_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${NEXIO_BROWSER_ORIGIN}" -H "Referer: ${NEXIO_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set NEXIO_COOKIE or NEXIO_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg paperclipApiUrl "$NEXIO_API_URL_FOR_OPENCLAW" \
...
persist_claimed_key_artifacts() {
  ...
  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    ...
    assert_status "200"
    {
      echo "NEXIO_API_URL=${NEXIO_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set NEXIO_API_KEY=${AGENT_API_KEY} in your run context before running Paperclip heartbeat steps."
...
main() {
  ...
  wait_http_ready "${NEXIO_API_URL%/}/api/health" 15 || fail "Paperclip API health endpoint not reachable"
