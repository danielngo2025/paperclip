...
NEXIO_HOST_PORT="${NEXIO_HOST_PORT:-3100}"
NEXIO_HOST_FROM_CONTAINER="${NEXIO_HOST_FROM_CONTAINER:-host.docker.internal}"
...
detect_paperclip_base_url() {
  ...
  for candidate in "$NEXIO_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${NEXIO_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${NEXIO_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${NEXIO_HOST_PORT}
  (Do not use http://127.0.0.1:${NEXIO_HOST_PORT} inside the container.)
