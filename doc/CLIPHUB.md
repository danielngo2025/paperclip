...
ClipHub is the public registry where people share, discover, and download Nexio company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Nexio what a package registry is to a programming language. Nexio already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Nexio — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Nexio instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Nexio spec. It contains:
...
**Install (fresh start):**
```
nexio install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Nexio instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Nexio, export your company as a template, then publish:

```
nexio export --template my-company
nexio publish cliphub my-company
```
...
ClipHub is a **separate service** from Nexio itself. Nexio is self-hosted; ClipHub is a hosted registry that Nexio instances talk to.
...
| **Nexio CLI** | `nexioai install`, `nexioai publish`, `nexioai cliphub sync` — built into Nexio |
| **Nexio UI** | "Browse ClipHub" panel in the Nexio web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Nexio) |
| Backend | TypeScript + Hono (consistent with Nexio) |
| Database | PostgreSQL |
...
4. Run `nexioai install cliphub:acme/lean-saas-shop`
5. Nexio creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Nexio until it works well
2. Export: `nexioai export --template my-agency`
3. Publish: `nexioai publish cliphub my-agency`
...
3. Install just that agent: `nexioai install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Nexio

ClipHub is **not required** to use Nexio. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Nexio what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `nexioai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Nexio web UI
- [ ] `nexioai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Nexio's own philosophy)
