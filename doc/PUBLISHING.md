...
Low-level reference for how Nexio packages are built for npm.
...
The CLI package, `nexioai`, imports code from workspace packages such as:
...
- `npx nexioai@canary onboard` can install them explicitly
- `npx nexioai onboard` continues to resolve `latest`
