...
This spec defines how Nexio actually runs agents while staying runtime-agnostic.
...
1. Nexio is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Nexio manages lifecycle, and on exit Nexio parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `nexio.skill` (shared Nexio skill text block)
...
2. Preferred transport is env vars (`NEXIO_*`) injected at runtime.
