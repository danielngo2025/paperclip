...
Target specification for the Nexio control plane. Living document — updated incrementally during spec interviews.

---

## 1. Company Model [DRAFT]

A Company is a first-order object. One Nexio instance runs multiple Companies. A Company does not have a standalone "goal" field — its direction is defined by its set of Initiatives (see Task Hierarchy Mapping).
...
Concepts like SOUL.md (identity/mission) and HEARTBEAT.md (loop definition) are **not part of the Nexio protocol**. They are adapter-specific configurations. For example, an OpenClaw adapter might use SOUL.md and HEARTBEAT.md files. A Claude Code adapter might use CLAUDE.md. A bare Python script might use command-line args.

Nexio doesn't prescribe how an agent defines its identity or behavior. It provides the control plane; the adapter defines the agent's inner workings.
...
#### Nexio Protocol (What Nexio Knows)

At the protocol level, Nexio tracks:
...
- **Fat payload** — Nexio bundles relevant context (current tasks, messages, company state, metrics) into the heartbeat invocation. Suited for simple/stateless agents that can't call back to Nexio.
- **Thin ping** — Heartbeat is just a wake-up signal. Agent calls Nexio's API to fetch whatever context it needs. Suited for sophisticated agents that manage their own state.

#### Minimum Contract

The minimum requirement to be a Nexio agent: **be callable.** That's it. Nexio can invoke you via command or webhook. No requirement to report back — Nexio infers basic status from process liveness when it can.

#### Integration Levels

Beyond the minimum, Nexio provides progressively richer integration:

1. **Callable** (minimum) — Nexio can start you. That's the only contract.
...
Nexio ships **default agents** that demonstrate full integration: progress tracking, cost instrumentation, and a **Nexio skill** (a Claude Code skill for interacting with the Nexio API) for task management. These serve as both useful defaults and reference implementations for adapter authors.
...
Agents can create tasks and assign them to agents outside their reporting line. This is the mechanism for cross-team collaboration. These rules are primarily encoded in the Nexio SKILL.md which is recommended for all agents. Nexio the app enforces the tooling and some light governance, but the cross-team rules below are mainly implemented by agent decisions.
...
The heartbeat is a protocol, not a runtime. Nexio defines how to initiate an agent's cycle. What the agent does with that cycle — how long it runs, whether it's task-scoped or continuous — is entirely up to the agent.

### Execution Adapters

Agent configuration includes an **adapter** that defines how Nexio invokes the agent. Initial adapters:
...
This is the full adapter contract. `invoke` starts the agent, `status` lets Nexio check on it, `cancel` enables the board's pause functionality. Everything else (cost reporting, task updates) is optional and flows through the Nexio REST API.
...
- Health monitoring — how does Nexio distinguish "stuck" from "working on a long task"?
...
Token/LLM cost budgeting is a core part of Nexio. External revenue and expense tracking is a future plugin.

### Cost Reporting

Fully-instrumented Agents report token/API usage back to Nexio. Costs are tracked at every level:
...
Nexio ships default Agent templates:

- **Default Agent** — a basic Claude Code or Codex loop. Knows the **Nexio Skill** (SKILL.md) so it can interact with the task system, read Company context, report status.
...
### Nexio Skill (SKILL.md)

A skill definition that teaches agents how to interact with Nexio. Provides:
...
The key constraint: it must be trivial to go from "I'm trying this on my machine" to "my agents are running on remote servers talking to my Nexio instance."

#### Agent Authentication

When a user creates an Agent, Nexio generates a **connection string** containing: the server URL, an API key, and instructions for how to authenticate. The Agent is assumed to be capable of figuring out how to call the API with its token/key from there.

Flow:

1. Human creates an Agent in the UI
2. Nexio generates a connection string (URL + key + instructions)
...
When a human completes a task, if the requesting agent's adapter supports **pingbacks** (e.g. OpenClaw hooks), Nexio sends a notification to wake that agent. This keeps humans rare but possible participants in the workflow.

The agents are discouraged from assigning tasks to humans in the Nexio SKILL, but sometimes it's unavoidable.
...
Nexio does **not** manage work artifacts (code repos, file systems, deployments, documents). That's entirely the agent's domain. Nexio tracks tasks and costs. Where and how work gets done is outside scope.
...
When an agent crashes or disappears mid-task, Nexio does **not** auto-reassign or auto-release the task. Instead:

- Nexio surfaces stale tasks (tasks in `in_progress` with no recent activity) through dashboards and reporting
- Nexio does not fail silently — the auditing and visibility tools make problems obvious
...
**Principle: Nexio reports problems, it doesn't silently fix them.** Automatic recovery hides failures. Good visibility lets the right entity (human or agent) decide what to do.

### Plugin / Extension Architecture

The core Nexio system must be extensible. Features like knowledge bases, external revenue tracking, and new Agent Adapters should be addable as **plugins** without modifying core. This means:
...
The plugin framework has shipped. Plugins can register new adapter types, hook into lifecycle events, and contribute UI components (e.g. global toolbar buttons). A plugin SDK and CLI commands (`nexioai plugin`) are available for authoring and installing plugins.
...
**Full loop with one adapter.** V1 must demonstrate the complete Nexio cycle end-to-end, even if narrow.
...
- [ ] **Default agent** — basic Claude Code/Codex loop with Nexio skill
- [ ] **Default CEO** — strategic planning, delegation, board communication
- [ ] **Nexio skill (SKILL.md)** — teaches agents to interact with the API
...
**Anti-goal for core.** The knowledge base is not part of the Nexio core — it will be a plugin. The task system + comments + agent descriptions provide sufficient shared context.
...
Things Nexio explicitly does **not** do:

- **Not an Agent runtime** — Nexio orchestrates, Agents run elsewhere
...
1. **Unopinionated about how you run your Agents.** Any language, any framework, any runtime. Nexio is the control plane, not the execution plane.
