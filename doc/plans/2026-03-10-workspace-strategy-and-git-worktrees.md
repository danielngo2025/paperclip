...
`PAP-447` asks how Nexio should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Nexio instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Nexio:

- some operators use Nexio to manage Nexio and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Nexio
- Claude and Codex expose different built-in affordances, so Nexio should not overfit to one tool

## Core Product Decision

Nexio should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Nexio
...
- Nexio should not assume that every adapter can see or use a host filesystem path directly
...
If Nexio makes worktrees agent-first, it will blur:
...
- Nexio core resolves and records execution workspace state
...
That gives Nexio room to support:
...
- Nexio handles workspace runtime services under the hood using project/workspace policy
...
Once Nexio is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Nexio cannot infer
...
- when issue moves to done, should Nexio auto-commit?
- should it auto-push?
...
Nexio already has the right foundation for a project-first model.
...
For server-spawned local adapters, Nexio already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Nexio should mirror directly

Implication:

- for `codex_local`, Nexio should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Nexio should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Nexio should separate:
...
Nexio should be able to express intentions such as:
...
These are not specific to Nexio. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".nexio/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Nexio-specific `devServer` flag.
...
Nexio should distinguish between:
...
Nexio should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Nexio should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Nexio
...
For runtime services specifically, Nexio should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Nexio-only dev-server flags
...
3. Inject resolved execution workspace details into `context.nexioWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Nexio instance can be reused through env/config injection when appropriate
...
- Nexio can decide whether to reuse or start a fresh service deterministically
...
3. Test against Nexio-on-Nexio workflows first.
...
7. Workspace runtime services are modeled generically, not as Nexio-specific dev-server toggles.
