...
1. In:
   ...
   inject `NEXIO_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `NEXIO_API_KEY`, do not overwrite it.
- Continue injecting:
  - `NEXIO_AGENT_ID`
  - `NEXIO_COMPANY_ID`
  - `NEXIO_API_URL`
...
- Local adapters authenticate without manual `NEXIO_API_KEY` config.
