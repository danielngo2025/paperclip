...
Add a single storage subsystem for Nexio that supports:
...
- API serves attachment bytes through authenticated Nexio endpoints.
...
- Default local path is under instance root: `~/.nexio/instances/<instanceId>/data/storage`.
...
- `nexioai onboard` writes a valid `storage` config block by default.
- `nexioai configure --section storage` can switch between local and s3 modes.
...
- `nexioai doctor` reports actionable storage status.
