...
- local mode that still feels instant (`npx nexioai run` and go)
...
2. CLI command `pnpm nexioai auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm nexioai onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm nexioai onboard`).
...
- expose key instance settings in API + CLI (`nexioai configure` / `nexioai onboard`)
...
7. `pnpm nexioai onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
