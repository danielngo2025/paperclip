...
Nexio needs to support two very different but equally valid ways of working:
...
Today, Nexio already has the beginnings of this model:
...
- making Nexio feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Nexio users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Nexio deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Nexio into a full code review product
...
This object must be recorded explicitly so that Nexio can:
...
Nexio should treat PRs as a type of work product linked back to:
...
Nexio already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Nexio must be a first-class deployment mode

Nexio cannot assume that it is running on the same machine as the code.

In cloud deployments, Nexio may:
...
- `Runtime service`: a process or service Nexio owns or tracks for a workspace
...
It also lets cloud-hosted Nexio deployments point at codebases and remotes without pretending the Nexio host has direct filesystem access.
...
Without an explicit `execution workspace` record, Nexio has nowhere stable to attach:
...
Nexio needs a single place to show:
...
- Nexio-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Nexio owning the execution host
...
## 1. Nexio host and execution host must be decoupled
...
without the Nexio host itself running local git or local preview processes.
...
That keeps Nexio focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Nexio deployments remain first-class

That is a better fit for Nexio than either extreme:
