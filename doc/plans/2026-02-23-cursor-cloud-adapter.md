...
This document defines the V1 design for a Nexio adapter that integrates with
...
4. synthesized stdout events for Nexio UI/CLI
...
3. **Agent callback auth to Nexio** uses a bootstrap exchange flow (no long-lived Nexio key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Nexio endpoints, not full SKILL.md prompt injection.
...
- `nexioPublicUrl` (optional override; else `NEXIO_PUBLIC_URL` env)
...
## Nexio Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `NEXIO_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.nexioPublicUrl`
2. `process.env.NEXIO_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Nexio credentials in prompt text.

Flow:

1. Before launch/follow-up, Nexio mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Nexio validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Nexio API calls.
...
1. Prompt includes a compact instruction to fetch skills from Nexio.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/nexio`
   - `GET /api/skills/nexio-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <nexioPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Nexio run context.
...
5. `nexioPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Nexio skill endpoints |
| Nexio auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Nexio context.
3. Add richer PR metadata surfacing in Nexio UI.
