...
Agents need API keys to authenticate with Nexio. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Nexio
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Nexio generates a token and passes it directly to the agent
...
`NEXIO_API_URL`, `NEXIO_AGENT_ID`, `NEXIO_COMPANY_ID`. We need to
add a `NEXIO_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Nexio CLI
...
```
nexio auth login
# Opens browser -> user confirms -> token stored at ~/.nexio/credentials
```
...
managed by the Nexio server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Nexio.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Nexio SKILL.md (or a link to it)
   - What information Nexio needs from the agent (e.g. webhook URL, adapter
...
5. Nexio stores the pending registration.
...
7. On approval, Nexio provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Nexio issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.nexio.dev/skills/nexio/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Nexio should send you heartbeats",
...
1. Generate a Nexio invite link for the company.
...
4. A Nexio company member approves the new agent.
5. Nexio begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `NEXIO_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `nexioai auth login` for developer-managed remote agents.                                      |
