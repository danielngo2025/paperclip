...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Nexio plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Nexio.
...
- package: `@nexioai/plugin-kitchen-sink-example`
- plugin id: `nexio.kitchen-sink-example` or `nexio-kitchen-sink-example`

Recommendation: use `nexio-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Nexio event such as `issue.created`
...
Recommendation: default to a Nexio-local endpoint or a stable public echo endpoint to avoid flaky docs.
