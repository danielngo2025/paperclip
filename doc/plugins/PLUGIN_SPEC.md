# Nexio Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Nexio's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Nexio app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Nexio HTTP APIs directly.
...
Nexio plugin design is based on the following assumptions:

1. Nexio is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Nexio business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Nexio core.
...
2. Let plugins add major capabilities without editing Nexio core.
...
4. Depend on project-local plugin folders such as `.nexio/plugins`.
...
The single Nexio deployment an operator installs and controls.

### 5.2 Company

A first-class Nexio business object inside the instance.
...
A trusted in-process extension loaded directly by Nexio core.
...
An installable instance-wide extension package loaded through the Nexio plugin runtime.
...
Nexio has two extension classes.
...
Nexio already has a concrete workspace model:
...
Plugins live under the Nexio instance directory.

Suggested layout:

- `~/.nexio/instances/default/plugins/package.json`
- `~/.nexio/instances/default/plugins/node_modules/`
- `~/.nexio/instances/default/plugins/.cache/`
- `~/.nexio/instances/default/data/plugins/<plugin-id>/`
...
Nexio should add CLI commands:

- `pnpm nexioai plugin list`
- `pnpm nexioai plugin install <package[@version]>`
- `pnpm nexioai plugin uninstall <plugin-id>`
- `pnpm nexioai plugin upgrade <plugin-id> [version]`
- `pnpm nexioai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@nexio/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface NexioPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that Nexio agents can use during runs.
...
- Nexio server starts one worker process per installed plugin
...
Receives one typed Nexio domain event.
...
export function definePlugin(definition: PluginDefinition): NexioPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: NexioPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@nexioai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@nexioai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@nexioai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@nexioai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@nexioai/plugin-sdk/ui`
...
The `@nexioai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Nexio secret provider system rather than a plain text input
...
1. Core Nexio data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Nexio objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Nexio product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Nexio later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Nexio secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm nexioai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Nexio server. This is a normative requirement, not optional.
...
### 27.1 `@nexioai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@nexioai/plugin-test-harness";
...
For developing a plugin against a running Nexio instance:

- The operator installs the plugin from a local path: `pnpm nexioai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-nexio-plugin`) that scaffolds:

- `package.json` with correct `nexioPlugin` keys
...
- `@nexio/plugin-workspace-files`
- `@nexio/plugin-terminal`
- `@nexio/plugin-git`
- `@nexio/plugin-linear`
- `@nexio/plugin-github-issues`
- `@nexio/plugin-grafana`
- `@nexio/plugin-runtime-processes`
- `@nexio/plugin-stripe`
...
- `@nexioai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@nexioai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@nexioai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@nexioai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@nexioai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@nexioai/plugin-sdk/ui`
...
- `@nexioai/plugin-test-harness`
- `create-nexio-plugin` starter template
...
Nexio should not implement a generic in-process hook bag modeled directly after local coding tools.

Nexio should implement:
...
That is the complete target design for the Nexio plugin system.
