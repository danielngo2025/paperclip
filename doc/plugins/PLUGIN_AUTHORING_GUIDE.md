...
This guide describes the current, implemented way to create a Nexio plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Nexio app.
...
```bash
pnpm --filter @nexioai/create-nexio-plugin build
node packages/plugins/create-nexio-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the Nexio repo:

```bash
pnpm --filter @nexioai/create-nexio-plugin build
node packages/plugins/create-nexio-plugin/dist/index.js @yourscope/plugin-name \
...
Inside this monorepo, the scaffold uses `workspace:*` for `@nexioai/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@nexioai/plugin-sdk` from the local Nexio checkout into a `.nexio-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Nexio from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@nexioai/plugin-sdk/ui`
