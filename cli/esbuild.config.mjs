/**
 * esbuild configuration for building the nexioai CLI for npm.
 *
 * Bundles all workspace packages (@nexioai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @nexioai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@nexioai/")) {
      externals.add(name);
    }
  }
