...
import { ApiRequestError, NexioApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Nexio config file")
    .option("-d, --data-dir <path>", "Nexio data directory root (isolates state from ~/.nexio)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Nexio API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.NEXIO_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const apiKey =
    options.apiKey?.trim() ||
    process.env.NEXIO_API_KEY?.trim() ||
    readKeyFromProfileEnv(profile);

  const companyId =
    options.companyId?.trim() ||
    process.env.NEXIO_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set NEXIO_COMPANY_ID, or set context profile companyId via `nexioai context set`.",
    );
  }

  const api = new NexioApiClient({ apiBase, apiKey });
  return {
    api,
    companyId,
    profileName,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.NEXIO_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.NEXIO_SERVER_PORT || "");
