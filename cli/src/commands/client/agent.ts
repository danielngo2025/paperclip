...
import type { Agent } from "@nexioai/shared";
import {
  removeMaintainerOnlySkillSymlinks,
  resolveNexioSkillsDir,
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export NEXIO_API_URL='${escaped(input.apiBase)}'`,
    `export NEXIO_COMPANY_ID='${escaped(input.companyId)}'`,
    `export NEXIO_AGENT_ID='${escaped(input.agentId)}'`,
    `export NEXIO_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Nexio skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Nexio skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveNexioSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Nexio skills directory. Expected ./skills in the repo checkout.",
              );
            }
