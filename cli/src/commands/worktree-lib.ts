...
export const DEFAULT_WORKTREE_HOME = "~/.nexio-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".nexio");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "nexio",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    NEXIO_HOME: paths.homeDir,
    NEXIO_INSTANCE_ID: paths.instanceId,
    NEXIO_CONFIG: paths.configPath,
    NEXIO_CONTEXT: paths.contextPath,
    NEXIO_IN_WORKTREE: "true",
    ...(branding?.name ? { NEXIO_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { NEXIO_WORKTREE_COLOR: branding.color } : {}),
  };
}
