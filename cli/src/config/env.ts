...
const JWT_SECRET_ENV_KEY = "NEXIO_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# Nexio environment variables",
    "# Generated by Nexio CLI commands",
...
export function resolveNexioEnvFile(configPath?: string): string {
...
export function loadNexioEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeNexioEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readNexioEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeNexioEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeNexioEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readNexioEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeNexioEnvEntries(next, filePath);
  return next;
}
