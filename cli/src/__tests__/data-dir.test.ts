...
describe("applyDataDirOverride", () => {
  beforeEach(() => {
    ...
    delete process.env.NEXIO_CONTEXT;
  ...
  it("sets NEXIO_HOME and isolated default config/context paths", () => {
    ...
    expect(process.env.NEXIO_HOME).toBe(expectedHome);
    expect(process.env.NEXIO_CONFIG).toBe(
      path.resolve(expectedHome, "instances", "default", "config.json"),
    );
    expect(process.env.NEXIO_CONTEXT).toBe(path.resolve(expectedHome, "context.json"));
    expect(process.env.NEXIO_INSTANCE_ID).toBe("default");
  });
  ...
  it("uses the provided instance id when deriving default config path", () => {
    ...
    expect(process.env.NEXIO_INSTANCE_ID).toBe("dev_1");
    expect(process.env.NEXIO_CONFIG).toBe(
  ...
  it("does not override explicit config/context settings", () => {
    process.env.NEXIO_CONFIG = "/env/config.json";
    process.env.NEXIO_CONTEXT = "/env/context.json";
    ...
    expect(process.env.NEXIO_CONFIG).toBe("/env/config.json");
    expect(process.env.NEXIO_CONTEXT).toBe("/env/context.json");
  });
  ...
  it("only applies defaults for options supported by the command", () => {
    ...
    expect(process.env.NEXIO_HOME).toBe(path.resolve("/tmp/paperclip-alt"));
    expect(process.env.NEXIO_CONFIG).toBeUndefined();
    expect(process.env.NEXIO_CONTEXT).toBeUndefined();
  });
});
