...
function buildSourceConfig(): NexioConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("builds isolated config and env paths for a worktree", () => {
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/nexio-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/nexio-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.NEXIO_HOME).toBe(path.resolve("/tmp/nexio-worktrees"));
    expect(env.NEXIO_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.NEXIO_IN_WORKTREE).toBe("true");
    expect(env.NEXIO_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.NEXIO_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export NEXIO_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nexio-worktree-secrets-"));
    const originalInlineMasterKey = process.env.NEXIO_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.NEXIO_SECRETS_MASTER_KEY_FILE;
    try {
      delete process.env.NEXIO_SECRETS_MASTER_KEY;
      delete process.env.NEXIO_SECRETS_MASTER_KEY_FILE;
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.NEXIO_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.NEXIO_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.NEXIO_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          NEXIO_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nexio-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.NEXIO_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.NEXIO_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".nexio-worktrees"),
      });

      const envPath = path.join(repoRoot, ".nexio", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("NEXIO_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("NEXIO_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/NEXIO_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.NEXIO_AGENT_JWT_SECRET;
      } else {
        process.env.NEXIO_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });

  it("defaults the seed source config to the current repo-local Nexio config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nexio-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".nexio", "config.json");
    const originalCwd = process.cwd();
    const originalNexioConfig = process.env.NEXIO_CONFIG;
    ...
    try {
      ...
      process.chdir(originalCwd);
      if (originalNexioConfig === undefined) {
        delete process.env.NEXIO_CONFIG;
      } else {
        process.env.NEXIO_CONFIG = originalNexioConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nexio-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nexio-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nexio-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    ...
    try {
      ...
      expect(fs.existsSync(path.join(worktreePath, ".nexio", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".nexio", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
