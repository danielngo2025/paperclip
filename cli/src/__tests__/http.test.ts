...
describe("NexioApiClient", () => {
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new NexioApiClient({
    ...
    expect(headers["x-nexio-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new NexioApiClient({ apiBase: "http://localhost:3100" });
