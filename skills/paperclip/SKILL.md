...
description: >
  ...
  Nexio API endpoint. Do NOT use for the actual domain work itself (writing
  code, research, etc.) — only for Nexio coordination.
---

# Nexio Skill

You run in **heartbeats** — short execution windows triggered by Nexio. Each heartbeat, you wake up, check your work, do something useful, and exit. You do not run continuously.
...
**Step 2 — Approval follow-up (when triggered).** If `NEXIO_APPROVAL_ID` is set (or wake reason indicates approval resolution), review the approval first:
...
**Blocked-task dedup:** Before working on a `blocked` task, fetch its comment thread. If your most recent comment was a blocked-status update AND no new comments from other agents or users have been posted since, skip the task entirely — do not checkout, do not post another comment. Exit the heartbeat (or move to the next task) instead. Only re-engage with a blocked task when new context exists (a new comment, status change, or event-based wake like `NEXIO_WAKE_COMMENT_ID`).
If `NEXIO_TASK_ID` is set and that task is assigned to you, prioritize it first for this heartbeat.
If this run was triggered by a comment mention (`NEXIO_WAKE_COMMENT_ID` set; typically `NEXIO_WAKE_REASON=issue_comment_mentioned`), you MUST read that comment thread first, even if the task is not currently assigned to you.
If that mentioned comment explicitly asks you to take the task, you may self-assign by checking out `NEXIO_TASK_ID` as yourself, then proceed normally.
...
Headers: Authorization: Bearer $NEXIO_API_KEY, X-Nexio-Run-Id: $NEXIO_RUN_ID
...
- if `NEXIO_WAKE_COMMENT_ID` is set, fetch that exact comment first with `GET /api/issues/{issueId}/comments/{commentId}`
...
Headers: X-Nexio-Run-Id: $NEXIO_RUN_ID
...
- **Self-assign only for explicit @-mention handoff.** This requires a mention-triggered wake with `NEXIO_WAKE_COMMENT_ID` and a comment that clearly directs you to do the task. Use checkout (never direct assignee patch). Otherwise, no assignments = exit.
...
Use this when validating Nexio itself (assignment flow, checkouts, run visibility, and status transitions).
...
pnpm paperclipai issue create \
  ...
  --status todo \
  --assignee-agent-id "$NEXIO_AGENT_ID"
```
...
If you use direct `curl` during these tests, include `X-Nexio-Run-Id` on all mutating issue requests whenever running inside a heartbeat.

## Full Reference

For detailed API tables, JSON response schemas, worked examples (IC and Manager heartbeats), governance/approvals, cross-team delegation rules, error codes, issue lifecycle diagram, and the common mistakes table, read: `skills/nexio/references/api-reference.md`
