---
name: nexio-create-plugin
description: >
  Create new Nexio plugins with the current alpha SDK/runtime. Use when
...
# Create a Nexio Plugin

Use this skill when the task is to create, scaffold, or document a Nexio plugin.
...
node packages/plugins/create-nexio-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Nexio repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.nexio-sdk/`:

```bash
pnpm --filter @nexioai/create-nexio-plugin build
node packages/plugins/create-nexio-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Nexio from an absolute local path during development
